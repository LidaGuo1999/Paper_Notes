...
- **3) Adversarial distance**：上面hardness measure是在对抗样本生成完毕后计算出来的，作者提出用一种简便的方法就可以预计出样本$X$到类别$t$的距离，定义为Adversarial distance，记作$A(X,t)$。
  $$
  A(X,t)=1-\frac{1}{M}\sum_{i\in 0..M}I_{S(X,t)[i]>0}
  $$
  其中$I_E$函数为指示函数，当条件$E$成立时取1，否则取0。显然，当值越接近1，代表ASM中可攻击的分量就越少，那么就越难被攻击为类别t，防御性能就越好。在此基础上，本文又提出了衡量模型$\mathbf{F}$鲁棒性的定义：
  $$
  R(\mathbf{F})=\min_{X,t}A(X,t)
  $$

#### C. Study of human perception of adversarial samples

该部分作者主要研究人眼对于上述对抗样本的识别程度。

第一个实验探究distortion对于人们判断的影响，结论是在某一个阈值以下（实验中为$\epsilon=14.29\%$时，人们的判断基本正确。一旦超过这个阈值，则正确率迅速下跌。

![](Pics/屏幕截图 2021-10-22 172224.png)

第二个实验探究单个像素扰动幅度大小（即算法中的$\theta$）对人们判断的影响，结论是不同的单个像素扰动幅度变化对实验结果影响不大，人们都能正确地进行分辨。

![](Pics/屏幕截图 2021-10-22 172344.png)

### 6 Discussion

- 作者认为本文提出的方法在任何满足feedforward特性的网络上都可以使用（比如循环神经网络不行），不仅可以用于监督学习，也可以用于非监督学习，只是具体的形式和评估需要相应调整；
- 作者提出两种方法进行对抗防御：
  - detection。在实验中发现，对抗样本中相邻两个像素点之间的平均平方距离均要大于原始样本，但这有可能只适用手写数字识别场景；
  - 提高robustness。作者也证明了使用对抗样本可以更好地帮助模型正则化，提高鲁棒性。利用对抗样本训练地模型准确率下降7.2%，distortion增加37.5%，可以说效果十分显著。
...
- explore adversarial behavior。本文对不同的对抗器进行了分类，根据它们所能掌握的信息和能力下了明确清晰的定义；
- 提出了一种基于前向导数的更简便构造对抗样本的方法；
- 提出未来对抗攻击防御的两个方向，分别为监测发现与提高模型的鲁棒性，并给出了一定的实验；

未来，作者打算继续深入，研究大致两个方向的问题。第一个是DNN在非监督学习下的局限性（本文是监督学习下的局限性），第二个是研究在有环网络（如循环神经网络）上的DL的局限性，这就和本文中所提到的前向导数有很大的区别了。
